...
The illustrated [web interface user guide](docs/user-guide/README.md) describes the interface. The scanner screenshots were refreshed from the experimental `v0.1.3` UI on 2026-08-29; labels, counters and example values can change. The [packet and frame analysis flow](docs/architecture/packet-analysis-flow.md) explains the L2/L3 ingress and independent passive-analysis paths.

**Independent passive modules:** [IDS, Network Presence & CVE Signatures](docs/user-guide/passive-detection.md) share one three-panel page with separate enable switches. Changes apply without rebooting. Presence can learn with IDS off, and signature matching can run independently of both. Disabling a module preserves its stored data. The guide's new passive-page screenshots use simulated lab data, not an on-device test.
...
| Configuration and evidence policy | Sectioned draft-based configuration editor with validation, revision protection, secret redaction and dedicated action-log files.                                   | [Configuration editor](docs/user-guide/configuration.md), [Log file management](docs/user-guide/logging.md) |
